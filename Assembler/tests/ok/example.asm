...
startLoop:
  lri r8,0x1230      ; peripheral's status register address, held in r8 for loop
  lri r9,0x1234      ; peripheral's data register address, held in r9 for loop
  lri r10,0x0001     ; bit to test for receive data present (1 = data is present in receiver)
  lri r11,0x0002     ; bit to test for transmit buffer empty (1 = transmitter ready to take data)
...
waitForTxReady:
  ...
  add r5,r9,r0       ; set the peripheral address register to the peripheral data register address
  stp r13            ; store the data that was read from the receiver into the transmitter
