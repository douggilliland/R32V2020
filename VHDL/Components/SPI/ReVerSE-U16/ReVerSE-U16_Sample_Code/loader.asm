...
; -----------------------------------------------------------------------------
; 06.04.2015	first version


system_port	equ #0001	; bit2 = (0:Loader ON, 1:Loader OFF); bit1 = (NC); bit0 = (0:M25P16, 1:ENC424J600)
mask_port	equ #0000	; Port mask EXT_MEM_PORT по AND
ext_mem_port	equ #dffd	; Memory port
pr_param	equ #7f00
...
;	ld (hl),a
	...
	ld bc,mask_port
	ld a,%11111111	; port mask по and
	out (c),a
	ld a,%00100100
	ld bc,ext_mem_port
	out (c),a

	xor a		; open the ozu page
spi_loader3
	ld bc,#7ffd
	out (c),a
	ld hl,#c000
...
spi_loader2
	...
	ld bc,ext_mem_port
	out (c),a
	ld a,%00011111	; port mask (allow 4mb)
	ld bc,mask_port
	out (c),a
	...
	ld hl,str3	;completed
	call print_str

	ld hl,str4	;initialization MC14818A
	call print_str
...
; #03: Command Register (write)
;	bit 7-1	= Reserved
;	bit 0	= 1:END   	(Deselect device after transfer/or immediately if START = '0')
; #03: Status Register (read):
; 	bit 7	= 1:BUSY	(Currently transmitting data)
;	bit 6-0	= Reserved

spi_end				; End a SPI transfer
	ld a,%00000001	; config = end
	out (#03),a		; write to command register
	ret
spi_start			; Start a SPI transfer
	xor a
	out (#03),a		; write to command register
	ret
spi_w
	in a,(#03)		; read status
	rlca
	jr c,spi_w		; loop while busy
	ld a,d
	out (#02),a		; write to SPI data out buffer
	ret
spi_r
	ld d,#ff		; write 0xff to the SPI data out buffer
	call spi_w
spi_r1	
	in a,(#03)		; read status
	rlca
	jr c,spi_r1		; loop while busy
	in a,(#02)
	ret
...
print_char
	...
	jr z,pchar2
	sub 32
	ld c,a			; temporarily save в с
	ld hl,(pr_param)	; hl=yx
	;coordinates -> scr adr
	;in: H - Y coordinate, L - X coordinate
	...
	or #58
	ld h,a
	ld a,(pr_param+2)	; Colour
	ld (hl),a		; print character attribute
	ld e,l
	ld l,c			; l= symbol
	ld h,0
	add hl,hl
...
;Key Waiting
anykey
	xor a			;all bits reset
	in a,(#fe)		;we survey all half-ranks
	cpl			;almost equivalent to combination:
	and 31 			;AND 31:CP 31,  but shorter
	jr z,anykey 		;until they click ANY KEY
	ret

;control codes
;13 (0x0d)			- after storm
;17 (0x11),color	- change the color of subsequent characters
;23 (0x17),x,y		- change position to coordinates x,y
;24 (0x18),x		- change position by x
;25 (0x19),y		- change position by y
;0					- end of line
